defmodule SelectoDBDuckDB.WriteCompiler do
  @moduledoc false
  ...
  def preview(%Batch{commands: commands}, opts) do
    with {:ok, statements} <- map_commands(commands, &compile(&1, opts)) do
      {:ok, %Preview{statements: statements, metadata: %{dialect: :duckdb, atomic?: true}}}
    end
  end

  def preview(%Command{} = command, opts) do
    with {:ok, statement} <- compile(command, opts) do
      {:ok, %Preview{statements: [statement], metadata: %{dialect: :duckdb}}}
    end
  end
  ...
  def compile_predicate(predicate, _opts, _offset) do
    {:error,
     Error.new(:invalid_predicate, "unsupported portable DuckDB predicate",
      ...
      case compile_value(value, opts, offset) do
        {:ok, value} ->
          assignment = %{
            ...
            text: cast_assignment(value.text, field, opts, value.params),
            params: value.params
          }
  ...
  defp compile_foreign_key_guards(metadata, assignments) do
    ...
    |> Enum.reduce_while({:ok, [], [], assignment_parameter_count(assignments)}, fn guard,
      ...
      with %{field: field, relation: relation, target_field: target_field} <- guard,
        ...
        guard_value =
          cast_assignment("$#{offset + 1}", field,
            field_types: Map.get(metadata, :field_types, %{})
          )

        text =
          "EXISTS (SELECT 1 FROM #{quote_relation(relation)} WHERE " <>
            "#{quote_identifier(target_field)} = #{guard_value})"
  ...
  defp inferred_parameter_type([value]) when is_integer(value), do: "BIGINT"
  defp inferred_parameter_type([value]) when is_float(value), do: "DOUBLE"
  defp inferred_parameter_type([value]) when is_boolean(value), do: "BOOLEAN"
  defp inferred_parameter_type([value]) when is_binary(value), do: "VARCHAR"
  defp inferred_parameter_type([%Date{}]), do: "DATE"
  defp inferred_parameter_type([%NaiveDateTime{}]), do: "TIMESTAMP"
  defp inferred_parameter_type([%DateTime{}]), do: "TIMESTAMPTZ"
  defp inferred_parameter_type([%{__struct__: Decimal}]), do: "DECIMAL"
  defp inferred_parameter_type(_params), do: nil

  defp fetch_field_type(types, field) when is_map(types) do
    Enum.find_value(types, fn {candidate, type} ->
      if to_string(candidate) == to_string(field), do: type
    end)
  end
  ...
  defp duckdb_type(type) when type in [:boolean, "boolean"], do: "BOOLEAN"
  defp duckdb_type(type) when type in [:date, "date"], do: "DATE"

  defp duckdb_type(type) when type in [:datetime, "datetime", :naive_datetime, "naive_datetime"],
    do: "TIMESTAMP"

  defp duckdb_type(type) when type in [:utc_datetime, "utc_datetime"], do: "TIMESTAMPTZ"
  ...
  defp renumber(text, delta) do
    Regex.replace(~r/\$(\d+)/, text, fn _whole, number ->
      "$#{String.to_integer(number) + delta}"
    end)
  end
